...
![作成フォーム](https://github.com/user-attachments/assets/cfc5f7ca-2f28-4d04-a618-4c0592555b87)
...
| 詳細設定 | 説明 |
| ------------- | ------------- |
| 設置可能ブロックID | minecraft:stoneなど、アイテムIDを設定します (,で区切ることで複数設定可能) |
| 破壊可能ブロックID | minecraft:stoneなど、アイテムIDを設定します (,で区切ることで複数設定可能) |
...
コピーして、使用することができます (execute as @a at @s run [command] でも使用可能)

# コマンド一覧
| コマンド  | 説明 |
| ------------- | ------------- |
| ci create | コマンドを作成します |
| ci edit [command] | コマンドを編集します (commandがない場合は、現在手に持っているアイテムが反映されます) |
| ci run [command] | コマンドを実行します |
